...
async function processPokerStart(roomCode, room, gameState, players) {
  ...
  if (n === 1) {
    ...
    const { data: pRow } = await supabase
      ...
      .select('inventory')
      .eq('room_id', room.id)
      .eq('client_id', firstPlayer)
      .single()
    const inv = (pRow?.inventory && typeof pRow.inventory === 'object') ? { ...pRow.inventory } : {}
    const chips = typeof inv.chips === 'number' ? inv.chips : 2000
    inv.chips = Math.max(0, chips - smallBlind)
    await supabase.from('players').update({ inventory: inv }).eq('room_id', room.id).eq('client_id', firstPlayer)
  } else if (n >= 2) {
