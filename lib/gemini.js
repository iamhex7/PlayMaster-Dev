...
export async function generateContentWithRetry(prompt, options = {}) {
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    // 找出这个key对应哪个环境变量（用于调试）
    const keySource = [
      process.env.GEMINI_KEY_1,
      process.env.GEMINI_KEY_2,
      process.env.GEMINI_KEY_3
    ].indexOf(currentKey)
    const keyLabel = keySource >= 0 ? `Key-[GEMINI_KEY_${keySource + 1}]` : `Key-[#${currentIndex + 1}]`
    ...
    try {
      ...
      if (isRetryableError(err)) {
        ...
        if (isQuotaExceededError) {
          console.error(`[Gemini API] ${keyLabel} 配额超限详情:`, {
            message: err.message,
            status: err.status,
            suggestion: '请检查该 API Key 的配额状态：https://ai.dev/rate-limit'
          })
        }
  ...
  // 生成更友好的错误信息
  const isAllQuotaExceeded = lastError && isQuotaExceeded(lastError)
  let errorMessage = ''
  
  if (isAllQuotaExceeded) {
    errorMessage = `❌ API 配额超限\n\n所有配置的 API Key 的免费层配额都已用完。\n\n解决方案：\n1. 检查配额状态：https://ai.dev/rate-limit\n2. 升级到付费计划或等待配额重置\n3. 使用新的、有配额的 API Key\n\n错误详情：${lastError?.message?.split('\n')[0] || '配额超限'}`
  } else {
    errorMessage = `API 调用失败：所有配置的 Key 都已尝试但全部失败。\n\n最后错误：${lastError?.message || '未知错误'}`
    if (lastError && isRetryableError(lastError)) {
      errorMessage += `\n\n提示：这是临时性错误，请稍后重试。`
    }
  }
